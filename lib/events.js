;(function(window) {
  ...
  if (window) {
    ...
    if (!('Calendr' in window)) {
      throw new Error("calendr.js is required");
    }
  ...
  function dailyRecurring(cal, event) {
    ...
    var startson = event.startson;
    var repeatendson = event.repeatEndson;
    var cmonth = calendar.month();
    var smonth = startson.month();
    var offset = startson.date();

    var i = (cmonth > smonth) ? 1 : offset;

    if (!!event.repeatTimes) {
      len = i+event.repeatTimes-1;

      // ensure len actually spans into the next month
      if (cmonth > smonth && len+offset <= startson.daysInMonth()) {
        return [];
      }
      ...
      if (len > numofdays) {
        ...
        while(cmonth > smonth) {
          var daysinmonth = moment([_calendarYear, (cmonth-1), 1]).daysInMonth();
      ...
      len = !outofBounds('month', cal, repeatendson) ? repeatendson.date() : numofdays+1;
    }
  ...
  function weeklyRecurring(cal, event) {
    ...
    var cmonth = calendar.month();
    var smonth = startson.month();
    var offset = indexOfWeek(startson);

    var i = (cmonth > smonth) ? 0 : offset;

    if (!!event.repeatTimes) {
      len = offset+event.repeatTimes-1;

      // ensure len actually spans into the next month
      if (cmonth > smonth && len < numberOfWeeks(startson)) {
        return [];
      }
      ...
      if (len > numofweeks) {
        var _calendarYear = calendar.year();
        while(cmonth>smonth) {
          var m = moment([_calendarYear, cmonth, 1]);
      ...
      len = !outofBounds('month', cal, repeatendson) ? indexOfWeek(repeatendson) : numofweeks-1;
    }
    ...
    for(; i<=len; i++) {
      ...
      for(; p<r; p++) {
        ...
        if (!outofBounds('month!', cal, startson) && date < startson.date()) {
          continue;
        }

        // remove dates after to the repeate ends on date
        if (!outofBounds('year', cal, repeatendson)) {
          if ((!outofBounds('month!', cal, repeatendson) && date > repeatendson.date()) || 
            !outofBounds('month!', cal, repeatendson, '>')) {
  ...
  function monthlyRecurring(cal, event) {
    var dates = [];
    var calendar = cal.moment;
    var startson = event.startson;
    var repeatendson = event.repeatEndson;
    ...
    if (!!event.repeatTimes) {
      var maxmonth = startson.month()+event.repeatTimes;
      if (calendar.month() < maxmonth) {
        dates = [startson.date()];
      }
    } else {
      if (!!!repeatendson ||                                  // infinite
          !outofBounds('month!', cal, startson) ||            // cal month same as start month
          !outofBounds('month!', cal, repeatendson, '<') ||   // cal month less than end month
          (!outofBounds('month!', cal, repeatendson) &&       // cal month same as start month &&
           repeatendson.date() >= startson.date())) {         // & ends on date is >= start date
  ...
  function yearlyRecurring(cal, event) {
    var dates = [];
    var calendar = cal.moment;
    var startson = event.startson;
    var repeatendson = event.repeatEndson;

    if (outofBounds('month!', cal, startson)) {
      return dates;
    }

    if (!!event.repeatTimes) {
      var maxyear = startson.year()+event.repeatTimes;
      if (calendar.year() < maxyear) {
        dates = [startson.date()];
      }
    } else {
      if (!!!repeatendson ||                              // infinite
          (calendar.year() <= repeatendson.year() &&      // cal year <= end year
           !outofBounds('month!', cal, repeatendson) &&   // & cal month same as end month
           repeatendson.date() >= startson.date())) {     // & ends on date is >= start date
